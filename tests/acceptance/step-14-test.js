...
test("when a song is loaded and the player is not playing, the component shows the song name", function(assert) {
  ...
  assert.equal(this.$().find('.play span:contains(▶)').length, 1, "The component should contain a play button");
  assert.equal(this.$().find('.play span:contains(❙❙)').length, 0, "The component should not contain a pause button");
  assert.equal(this.$().find('.now-playing-name').text().trim(), 'Song name', "The component shows the song name");
});
