...
test("The helper correctly formats number of seconds into human-readable strings", function(assert) {
  let { compute } = resolver.resolve('helper:format-duration');
  assert.ok(compute, "The format-duration helper exists");

  assert.equal(compute([0]), '0:00', "0 is converted into 0:00");
  assert.equal(compute([8]), '0:08', "Less than 10 seconds gets 0-padded");
  assert.equal(compute([20]), '0:20', "Numbers with trailing zeros are not truncated");
  assert.equal(compute([60]), '1:00', "Exactly 60 seconds is converted into 1:00");
  assert.equal(compute([61]), '1:01', "Less than 10 seconds into a minute gets 0-padded");
  assert.equal(compute([70]), '1:10', "Numbers with trailing zeros greater than one minute are not truncated");
  assert.equal(compute([125]), '2:05', "The helper works with multiple minutes");
});
