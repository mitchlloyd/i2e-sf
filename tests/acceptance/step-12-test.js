...
test("Playing a song should update the now-playing template", function(assert) {
  ...
  andThen(function() {
    ...
    assert.ok(exists('.now-playing div.now-playing-body span.now-playing-name:contains(A Walk)'), "The current song's name is showing");
    assert.ok(exists('.now-playing div.now-playing-body span.now-playing-artist:contains(GOLDHOUSE)'), "The current song's artist is showing");
  });
});
