/*
  For this step, use the `{{#if}}` helper to only show an "EXPLICIT" warning
...
function albumIsExplicit(name) {
  let el = findWithAssert('.album:has(.album-name:contains('+name+'))');
  return el.find('.explicit:contains(EXPLICIT)').length === 1;
}

step(2, "Add Explicit Warning");

test("Explicit albums should have a warning about their contents", function(assert) {
  visit('/');
  ...
  andThen(function() {
    ...
    assert.ok(albumIsExplicit('The Heist'), "Third album should be explicit");
    assert.ok(albumIsExplicit('Some Nights'), "Fourth album should be explicit");
  });
});
